...
const NewProfile = () => {
  ...
  const handleSelectedId = async (selectedId) => {
   // setEducation(newCompanies);
    ...
    const experience = await pb.collection('Experience').getFirstListItem(`candidate="${selectedId}"`);
    record.companies = experience;
    
    const education = await pb.collection('Education').getList(`candidate="${selectedId}"`);
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
     ...
     <div className="form-group column">
      ...
      <div className="form-group column">
        <label htmlFor="documentUpload">UAN Document Upload:</label>
        <input
          type="file"
          id="documentUpload"
          name="documentUpload"
